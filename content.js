...
async function Bot(inputText) {
    if (CONFIG.USE_OPENAI) {
        return botOpenAI(inputText);
    } else {
        return botGemini(inputText);
    }
}

async function botGemini(inputText) {
    const payload = {
        contents: [
            {
                parts: [
    ...
    return new Promise((resolve, reject) => {
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    try {
                        const geminiData = JSON.parse(xhr.responseText);
...
async function botOpenAI(inputText) {
    const payload = {
        model: "gpt-3.5-turbo",
        messages: [
            {
                role: "system",
                content: "You are a helpful assistant that summarizes text. Try not to miss any important details regarding the core topic of the post."
            },
            {
                role: "user",
                content: `Summarize this post: ${inputText}`
            }
        ],
        max_tokens: 150
    };
    ...
    return new Promise((resolve, reject) => {
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    try {
                        const openAIData = JSON.parse(xhr.responseText);
                        const textOutput = openAIData.choices[0].message.content.trim();
                        resolve(textOutput);
                    } catch (error) {
                        console.error('Error parsing JSON:', error.message || error);
                        reject(error);
